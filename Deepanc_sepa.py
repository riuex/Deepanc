...
class convert_main:
    ...
    def run(self):
        ...
        self.level = self.simage.level_count - 1
        self.Xmax,self.Ymax = self.simage.level_tiles[self.level]
        self.Xmax,self.Ymax = self.Xmax - 1,self.Ymax - 1
        self.conunter = 0
        p = Pool(self.coreN)
        for j in range(self.Ymax):
            dataList = [(self.image_path,self.pixels,self.overlap,self.limit_bounds,i,j,self.Xmax,self.Ymax,self.level,self.border,self.save_name,self.conunter) for i in range(self.Xmax)]
            print("\n processing" + str(j) + "\n")
            p.map(hub,dataList)
        print("mission completed!! Good bye!!!!")
            #print("Number of tiles : " + str(conunter) + "/" + str(Xmax*Ymax))

    def sub(image_path,pixels,overlap,limit_bounds,x,y,Xmax,Ymax,level,border,save_name,counter):
