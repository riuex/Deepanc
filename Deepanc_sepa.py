...
class convert_main:
    ...
    def run(self):
        self.image = open_slide(self.image_path)
        ...
        self.simage = DeepZoomGenerator(self.image,self.pixels,self.overlap,self.limit_bounds)
        level = self.simage.level_count - 1
        self.Xmax,self.Ymax = self.simage.level_tiles[level]
        self.Xmax,self.Ymax = self.Xmax - 1,self.Ymax - 1
        conunter = 0
        p = Pool(coreN)
        for j in range(Ymax):
            dataList = [(self.image_path,self.pixels,self.overlap,self.limit_bounds,i,j,self.Xmax,self.Ymax,self.level,self.border,self.save_name,self.conunter) for i in range(Xmax)]
            print("\n processing" + str(j) + "\n")
            p.map(hub,dataList)
    ...
    def hub(self,dataList):
        sub(*dataList)

    def sub(self,image_path,pixels,overlap,limit_bounds,x,y,Xmax,Ymax,level,border,save_name,counter):
...
if __name__ == "__main__":
    ...
    if function == "conv3pro":
        ...
        convert_main(image_path,args.output,widepixels,1,False,args.multi,args.border).run()
    else :
